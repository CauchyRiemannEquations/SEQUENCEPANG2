...
const jevOption = new URLSearchParams(location.search).get('jev');
const jevMode = jevOption ?? 'active';
...
function help(active = 'arithmetic') {
  ...
  show(`<h2 id="modal-title">플레이 방법</h2>
    ...
    <p class="help-note">온라인에서는 플레이 요약을 TypeSafe에 보내 힌트를 준비해요. 이름이나 터치 경로는 보내지 않아요.</p>
...
if (observation && ['1','active'].includes(jevOption)) import('./jev-debug.js').then(({mountJevDebug}) => mountJevDebug({
