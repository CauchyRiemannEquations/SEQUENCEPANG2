...
// Bounded hint-only requests. Limits are per instance, not a distributed quota.
// Missing credentials or provider failures leave deterministic hints available.
export function createHandler({env = process.env, fetcher = (...args) => fetch(...args), now = Date.now} = {}) {
  ...
  return async function handler(req, res) {
    ...
    if (!['production','preview','development'].includes(env.VERCEL_ENV)) return send(404,{error:'environment_disabled'});
