...
test('Jev requires a deployment environment and key; production config GET is free',async()=>{
  let count=0;
  const fetcher=async()=>{count++;throw Error('should not call');};
  for(const env of [{}]) assert.equal((await call(createHandler({env,fetcher}))).statusCode,404);
  assert.equal((await call(createHandler({env:{VERCEL_ENV:'preview'},fetcher}))).statusCode,503);
  const r=await call(createHandler({env:{VERCEL_ENV:'production',TYPESAFE_API_KEY:'test'},fetcher}),{method:'GET'});
...
test('valid request uses fixed server questions; response is shadow-only and secret-free',async()=>{
  let received;
  const handler=createHandler({env:{VERCEL_ENV:'production',TYPESAFE_API_KEY:'private-test-key'},fetcher:async(url,options)=>{
